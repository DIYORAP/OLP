...
const Performance = () => {
    ...
    const getInstructorDashboard = async (token, dispatch) => {
        const toastId = toast.loading("Loading...");
        let result = []
        try {
            console.log("BEFORE Calling BACKEND API FOR INSTRUCTOR DASHBOARD");
            const response = await axios.get("/api/profile/getInstructorDashboardDetails",
                null,
                {
                    Authorisation: `Bearer ${token}`,
                }
            ...
            <div className='mx-auto w-11/12 max-w-[1000px] py-10'>
                <div>
                    <div className='space-y-2'>
                        <h1 className='text-2xl font-bold text-richblack-5'>Hi {user?.username} 👋</h1>
