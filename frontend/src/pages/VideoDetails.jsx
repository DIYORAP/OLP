...
import { setCompletedLectures } from '../redux/Slice/viewCourseSlice';
...
const VideoDetails = () => {
    ...
    const markLectureAsComplete = async (data, token) => {
        ...
        try {
            const response = await axios.post("/api/courses/updateCourseProgress", data, {
    ...
    return (
        <div className='md:w-[calc(100vw-320px)] w-screen p-3'>
                                            ...
                                            <div className='flex justify-center items-center'>
                                                {
                                                    !completedLectures.includes(videoData._id) && (
                                                        <button onClick={() => { handleLectureCompletion() }} className='bg-black text-white absolute top-[20%] hover:scale-90 z-20 font-medium md:text-sm px-4 py-2 rounded-md'>Mark as Completed</button>
                                                    )
                                                }
                                            </div>
                                            {
            ...
            <div className='mt-5'>
                ...
                <p className='text-gray-700 text-richblack-100'>{videoData?.description}</p>
            </div>
        </div>
    )
}
