...
import CourseInformationForm from './CouresInfromation/CourseInformationForm';
import CourseBuliderForm from './CourseBulider/CourseBuliderForm';
                            ...
                            <div key={item.id} className=' flex w-full justify-between'>
                                <div className='flex flex-col items-center'>
                                    <div className={`grid cursor-default aspect-square w-[34px] place-items-center rounded-full border-[1px] ${step === item.id
                                        ...
                                        : "border-black bg-white text-richblack-300"}`}>
                                        {
                                ...
                                {item.id < 3 && (
                                    <div className={`h-[calc(34px/2)] w-[100%]  border-dashed border-b-2 ${step > item.id ? "border-green-700" : "border-black"}
                    }`}></div>
                                )}
                            </div>
                        ))}
